...
const STAGE_DEFS = [
  { id: 1, name: 'Arrival', description: 'Enter Walden Woods and name your purpose', icon: '🌲' },
  { id: 2, name: 'First Cards', description: 'Complete your first card deck', icon: '🃏' },
  { id: 3, name: 'First Action', description: 'Log your first required action', icon: '✋' },
  { id: 4, name: 'Character', description: 'Unlock your Totemic Character', icon: '🦉' },
  { id: 5, name: 'Train Station', description: 'Access Train Station resources', icon: '🚉' },
  { id: 6, name: 'Forum', description: 'Post in a Territory forum', icon: '💬' },
  { id: 7, name: 'Cross-App', description: 'Link your journey to Walditorium or Harvest Trees', icon: '🔗' },
];

export function JourneyPage() {
  const navigate = useNavigate();
  const { journeyStage, territories } = useStore();

  const stages = STAGE_DEFS.map((def, i) => ({ ...def, complete: i < journeyStage }));
  const nextStage = stages.find(s => !s.complete);
  const completedCount = stages.filter(s => s.complete).length;
  const visitedTerritories = territories.slice(0, 2);
  ...
  return (
    <div style={{ background: '#0a1509', minHeight: '100vh', color: '#f5e6c8', padding: '48px 40px' }}>
      ...
      <h1 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '40px', fontWeight: 600, margin: '0 0 40px' }}>
        Stage {completedCount} of {STAGE_DEFS.length}
      </h1>
